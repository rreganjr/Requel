 ...
 */
@Component("stemmer")
public class Stemmer implements NLPProcessor<NLPText> {
	...
	public NLPText process(NLPText text) {
		...
		if (text.is(GrammaticalStructureLevel.WORD)) {
			((NLPTextImpl) text).setLemma(stemmer.stripAffixes(text.getText()));
		} else {
	...
	/*
	 * author: Fotis Lazarinis (actually I translated from C to Java) date: June
	 * 1997 address: Psilovraxou 12, Agrinio, 30100 comments: Compile it, import
	 * the Porter class into you program and create an instance. Then use the
	 * stripAffixes method of this method which takes a String as input and
	...
	static class PorterStemmer {
		...
		private String step2(String str) {
			...
			String[][] suffixes = { { "ational", "ate" }, { "tional", "tion" }, { "enci", "ence" },
					{ "anci", "ance" }, { "izer", "ize" }, { "iser", "ize" }, { "abli", "able" },
		...
		private String step3(String str) {
			...
			String[][] suffixes = { { "icate", "ic" }, { "ative", "" }, { "alize", "al" },
		...
		private String stripPrefixes(String str) {
			...
			String[] prefixes = { "kilo", "micro", "milli", "intra", "ultra", "mega", "nano",
					"pico", "pseudo" };
