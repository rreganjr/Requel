...
Create a stable assistant extension point that supports the existing NLP/lexical assistants, future rules-based assistants, and AI-backed assistants described in `doc/work/2.0/ai-assistance-plan.md`.
...
> are deferred to Phase 5+. See `doc/work/2.0/43-phase-4.5-plan.md` (Completion section) for the full
...
`assistant-api` and `assistant-core` are introduced together in the first implementation branch. `assistant-legacy-nlp`, `assistant-ai` (neutral contracts + Noop), `assistant-openai` (OpenAI client), and `mcp-server` follow in later phases per `doc/work/2.0/ai-assistance-plan.md` rollout.
...
The SPI is the foundation for `doc/work/2.0/ai-assistance-plan.md`:
...
Context pack DTOs (`ProjectContextPack`, `EntityContextPack`, `IssueContextPack` from `doc/work/2.0/ai-assistance-plan.md`) live in `assistant-core`, not `service-api`.
...
9. Add AI-backed assistants and MCP support following `doc/work/2.0/ai-assistance-plan.md`.

## Resolved Decisions

Resolved during issue #43 walkthrough. See the full record at <https://github.com/rreganjr/Requel/issues/43#issuecomment-4560006774>, and the AI-side decisions in `doc/work/2.0/ai-assistance-plan.md`.
