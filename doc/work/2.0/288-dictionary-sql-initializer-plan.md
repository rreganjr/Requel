...
5. **`doc/architecture/DICTIONARY_LOADING.md`** (new, AC 4) — which environments load via SQL (MySQL deployment,
...
- **The import list becomes a config surface.** `DictionarySQLInitializer.properties` ships inside
  ...
  and has to become `requel.dictionary.sql-files`. Noted in `doc/architecture/DICTIONARY_LOADING.md`.
