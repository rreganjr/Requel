...
`doc/guides/local_mcp_bridge.md`. Depends on the issue-tracker→Goals workflow ticket (series ticket 3).
...
- No external calls in v1; if embeddings are added later, follow the AI data-handling policy
  (opt-in, redaction, retention) from `doc/work/2.0/ai-assistance-plan.md`.
