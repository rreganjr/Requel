...
- `doc/work/2.0/assistant-spi-plan.md`
- `doc/work/2.0/ai-assistance-plan.md`
- `doc/work/2.0/nlp-optional-plan.md`
...
`doc/work/2.0/ai-assistance-plan.md` currently presents an in-process MCP endpoint in `service-impl` as the first useful path, while the resolved decision says to create a new `mcp-server` Maven module bundled in-process with `requel-app`.
...
`doc/work/2.0/nlp-optional-plan.md` says Requel should start when `nlp-jpa` is absent, but the fallback factory still depends on `NLPProcessorFactory`, which currently lives in `nlp-jpa`.
...
`doc/work/2.0/assistant-spi-plan.md` lists applicator mappings such as:
...
`doc/work/2.0/ai-assistance-plan.md` still says `service-api` / `service-impl` are "the right place to expose MCP-facing query/application services." Since MCP is now owned by `mcp-server`, this should probably say those modules provide the authenticated query/application services that `mcp-server` delegates to.
