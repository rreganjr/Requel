...
> Background/design: `doc/work/2.0/oauth_mcp_plan.md` (Slice 4/5 DCR decision), `doc/work/2.0/83-oauth-verification.md`
> (Part D — the trigger), `doc/guides/mcp_remote_connection.md`. Depends on #83 (embedded AS), #98
...
6. **Docs:** make OAuth the primary codex recipe in `doc/guides/mcp_remote_connection.md`; flip the Part D
   finding + "Anonymous-DCR shim needed?" decision in `doc/work/2.0/83-oauth-verification.md` to reflect codex
...
- **Manual (record in `doc/work/2.0/83-oauth-verification.md` Part D):** `codex mcp login requel` and
...
- SPA auth unification onto the embedded AS; external-IdP support (both deferred in
  `doc/work/2.0/oauth_mcp_plan.md`).
