...
Scope reviewed:
- `doc/work/2.0/UI_REFACTOR_PLAN.md`
- `doc/archive/IDENTITY_MIGRATION.md` (dependency alignment check)
- `doc/guides/RELEASE_GUIDE.md` (delivery/release alignment check)
...
- Evidence:
  - `doc/work/2.0/UI_REFACTOR_PLAN.md:943` says entities include `version` for optimistic locking.
...
- Evidence:
  - `doc/work/2.0/UI_REFACTOR_PLAN.md:941` defines list responses as `{ items, total, page, pageSize }`.
...
- Evidence:
  - Phases list work items but no explicit acceptance criteria (`doc/work/2.0/UI_REFACTOR_PLAN.md:575-881`).
...
- Evidence:
  - Coexistence is acknowledged (`doc/work/2.0/UI_REFACTOR_PLAN.md:577`) and Echo2 removal is planned (`:871-881`), but no rollback path is documented.
- Risk:
...
- Evidence:
  - UI plan relies on current user/password/role model (`doc/work/2.0/UI_REFACTOR_PLAN.md:553-559`, `:599`).
  - Identity plan introduces `platform-identity` and role/authority remapping (`doc/archive/IDENTITY_MIGRATION.md:14-15`, `:33-40`, `:71-84`).
- Risk:
...
- Evidence:
  - `doc/work/2.0/UI_REFACTOR_PLAN.md:447-448` acknowledges token exposure in URL/logs and accepts risk.
- Recommendation:
...
- Evidence:
  - Event bridge and SSE architecture are defined (`doc/work/2.0/UI_REFACTOR_PLAN.md:535-544`) but no metrics/alerts/logging plan.
- Recommendation:
...
- Evidence:
  - Relative-size estimates exist (`doc/work/2.0/UI_REFACTOR_PLAN.md:1031-1047`) without schedule/ownership.
- Recommendation:
...
- Optimistic locking is now specified with `version` fields and a `409 Conflict` contract (`doc/work/2.0/UI_REFACTOR_PLAN.md:1246-1262`), and edit/delete command inputs now include `version` across phases (examples: `:937`, `:962`, `:1012-1017`, `:1041-1045`, `:1072-1073`, `:1097`, `:1126-1129`).
- Query list response format is now standardized as a paginated envelope (`doc/work/2.0/UI_REFACTOR_PLAN.md:1242`).
- Cutover/rollback and phase Definition of Done were added (`doc/work/2.0/UI_REFACTOR_PLAN.md:1353-1369`).
- Identity migration sequencing/dependency notes were added (`doc/work/2.0/UI_REFACTOR_PLAN.md:1347-1351`).
- Authorization design is now explicitly documented and linked via `doc/architecture/AUTH_ARCH.md` (`doc/work/2.0/UI_REFACTOR_PLAN.md:859-861`).
...
- Evidence:
  - UI plan JWT claims are documented as `sub`, `roles`, `exp` (`doc/work/2.0/UI_REFACTOR_PLAN.md:287`, `:882`).
  - `AUTH_ARCH.md` includes `permissions` in JWT and Angular depends on it (`doc/architecture/AUTH_ARCH.md:427-429`, `:458-460`).
  - `AUTH_ARCH.md` requires `GET /api/projects/{projectId}/my-permissions` (`doc/architecture/AUTH_ARCH.md:440`, `:548`), but this endpoint is not listed in UI plan API conventions (`doc/work/2.0/UI_REFACTOR_PLAN.md:1192-1212`).
  - UI plan says all project features are visible to any authenticated user (`doc/work/2.0/UI_REFACTOR_PLAN.md:855`), while `AUTH_ARCH.md` defines fine-grained project permission gating in Angular (`doc/architecture/AUTH_ARCH.md:435-505`).
- Recommendation:
...
- Evidence:
  - `AUTH_ARCH.md` examples use URL-based admin rules for `/api/admin/**` (`doc/architecture/AUTH_ARCH.md:407`, `:556`).
  - UI plan uses command dispatch at `/api/commands/{commandType}` and user endpoints at `/api/users` (`doc/work/2.0/UI_REFACTOR_PLAN.md:1188`, `:1195`), not `/api/admin/**`.
- Risk:
...
- Evidence:
  - `AuthorizableCommand` extends `EditCommand` and defines only `getAuthorizationRequirement()` (`doc/architecture/AUTH_ARCH.md:115-122`).
  - `AuthorizingCommandHandler` uses `command.getEditedBy()` (`doc/architecture/AUTH_ARCH.md:173`), but the `EditCommand` snippet shows only `setEditedBy(User)` (`doc/architecture/AUTH_ARCH.md:74-76`).
- Recommendation:
...
- Evidence:
  - UI plan says no schema changes/migrations are needed for Angular frontend (`doc/work/2.0/UI_REFACTOR_PLAN.md:1354`).
  - Open question states `@Version` may require Flyway migrations (`doc/work/2.0/UI_REFACTOR_PLAN.md:1402`).
- Recommendation:
...
- Evidence:
  - Command-side mapping is described via `ExceptionMappingCommandHandler` (`doc/architecture/AUTH_ARCH.md:522`).
  - Query authorization throws `AuthorizationException` in `ProjectAccessChecker` (`doc/architecture/AUTH_ARCH.md:359-360`), but query paths do not traverse command handlers.
- Recommendation:
