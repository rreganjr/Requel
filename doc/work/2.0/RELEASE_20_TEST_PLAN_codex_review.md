...
`doc/work/2.0/RELEASE_20_TEST_PLAN.md:206-267`.
...
`doc/work/2.0/RELEASE_20_TEST_PLAN.md:206-225`, but there are active services for users
...
but `doc/work/2.0/RELEASE_20_TEST_PLAN.md:127-132` does not explicitly include them in the REST coverage.
...
- `AuthControllerTest` includes `POST /api/auth/logout` in
  `doc/work/2.0/RELEASE_20_TEST_PLAN.md:130`, but the controller only exposes `POST /api/auth/login` and
  `GET /api/auth/me`
...
- `EventStreamControllerTest` only covers the initial `GET /api/events/stream` call in
  `doc/work/2.0/RELEASE_20_TEST_PLAN.md:131`, but the real controller also includes `POST` and `DELETE`
...
- The stakeholder command matrix names `DeleteUserStakeholder` and `DeleteNonUserStakeholder` in
  `doc/work/2.0/RELEASE_20_TEST_PLAN.md:109`, but the registered command is `DeleteStakeholder`
...
- `auth.service.spec.ts` calls for `isLoggedIn()` and token-expiry behavior in
  `doc/work/2.0/RELEASE_20_TEST_PLAN.md:213`, but `AuthService` exposes `isAuthenticated` and does not
  parse JWT expiry
  (`requel-angular/src/app/core/auth.service.ts:31-75`).
- `project.service.spec.ts` calls for cache invalidation in
  `doc/work/2.0/RELEASE_20_TEST_PLAN.md:215`, but `ProjectService` currently has an event subject, not a
...
- `scenario-selector-dialog.spec.ts` calls for `[includeTypes]` coverage in
  `doc/work/2.0/RELEASE_20_TEST_PLAN.md:235`, but `ScenarioSelectorDialogComponent` only accepts
...
The command section in `doc/work/2.0/RELEASE_20_TEST_PLAN.md:102-117` omits glossary and report generator
...
For a release plan, the current E2E matrix in `doc/work/2.0/RELEASE_20_TEST_PLAN.md:327-419` is a good
...
the backend must be on `:8081` (`doc/work/2.0/RELEASE_20_TEST_PLAN.md:316` and `doc/work/2.0/RELEASE_20_TEST_PLAN.md:482`),
but the CI health check waits on `http://localhost:8080/actuator/health`
(`doc/work/2.0/RELEASE_20_TEST_PLAN.md:429-443`).
