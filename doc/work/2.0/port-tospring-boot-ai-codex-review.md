...
`doc/work/2.0/port-tospring-boot-ai.md` says Requel already meets Spring AI's baseline and that Spring AI `1.0.x` fits with "no upgrade needed" (lines 8 and 21-29). The root POM is on Spring Boot `3.3.4` and Java `17`.
...
The current plan says to pin Spring AI `1.0.x` and use `McpTransportContext` for stdio identity (`doc/work/2.0/issue_69_subtasks.md`, line 78; `doc/work/2.0/port-tospring-boot-ai.md`, lines 350-359). That is not consistent with the Spring AI docs:
...
`doc/work/2.0/port-tospring-boot-ai.md` says `.entity(ReviewResult.class)` "forces" the model output and removes hand validation (lines 92-100 and 135-137). `doc/work/2.0/issue_spring_ai_provider_clients.md` repeats this in acceptance criteria (lines 73-78).
...
`doc/work/2.0/ai-assistance-plan.md` still describes `OpenAiAnalysisClient` using the OpenAI Responses API as the initial implementation (lines 75-91 and 327-331). That conflicts with the new Spring AI provider-client issue, which plans a single `SpringAiAnalysisClient` over `ChatClient`.
...
`doc/work/2.0/issue_69_subtasks.md` says Spring AI handles authn and coarse per-tool gating (line 58). That is only safe if the implementation uses the right Spring Security setup and, for the community MCP security module, the right Spring AI line.
...
`doc/work/2.0/issue_69_subtasks.md` says `gateway-client-rest` is likely unnecessary if stdio runs in-process (lines 64-78). That may be true for a local developer profile, but it is not obviously true for external clients that need to talk to an already-running Requel server.
...
### `doc/work/2.0/port-tospring-boot-ai.md`
...
### `doc/work/2.0/issue_69_subtasks.md`
...
### `doc/work/2.0/issue_spring_ai_provider_clients.md`

- Fix the background path from `notes/port-tospring-boot-ai.md` to `doc/work/2.0/port-tospring-boot-ai.md`.
...
### `doc/work/2.0/ai-assistance-plan.md`
