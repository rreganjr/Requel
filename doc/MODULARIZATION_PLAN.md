...
|-------|------------------------------|-----------------------------|------------|
| Foundation | `platform-core` | Move shared exceptions (`RequelException`, `EntityValidationException`, `EntityLockException`, `NoSuchEntityException`), `Describable`, `NamedEntity`, `OrganizedEntity` (after decoupling from `User`), `CreatedEntity` (adjusted to reference a lightweight `UserReference`). Also relocate `SystemInitializer` & `AbstractSystemInitializer` to new `com.rreganjr.platform.bootstrap`. **Status:** already hosts `DomainObjectWrapper`, `EntityProxyInterceptor`, etc. | none |
| Foundation | `platform-identity` | Minimal identity API (`com.rreganjr.platform.identity.User/Role`), authentication services, password hashing, plus the legacy `com.rreganjr.requel.user.impl` implementation until the user-domain split lands. Provides `User2UserImplAdapter` bridging identity ↔ domain. **Status:** module exists; still contains user domain & JPA implementations that must migrate into dedicated modules. | `platform-core` |
...
| Foundation | `repository-core` | `com.rreganjr.repository` & `com.rreganjr.repository.jpa` minus domain-specific adapters, refactored to depend on interfaces (`UserSet`, etc.) instead of impl classes. **Status:** base JPA infrastructure migrated; identity-specific wrappers now extend from `platform-identity`. | `platform-core`, `command-core` |
| Domain | `user-domain` | `com.rreganjr.requel.user` interfaces, DTOs, exceptions. Convert `CreatedEntity` dependency to the new `UserReference` abstraction. **Status:** pending—interfaces currently reside in `platform-identity`. | `platform-core` |
| Domain | `user-jpa` | `com.rreganjr.requel.user.jpa` (renamed from `.impl`) entities and repository adapters. Depends on `user-domain`, `repository-core`. **Status:** pending—`UserImpl` and repositories remain in `platform-identity`. | `user-domain`, `repository-core` |
...
- `platform-core`, `platform-identity`, and `requel-app` modules already exist. Core JPA utilities and the generic `DomainObjectWrapper` live in `platform-core`; `UserAwareDomainObjectWrapper` and all legacy `com.rreganjr.requel.user.*` classes sit in `platform-identity` for now.
- The identity module still contains user domain entities (`UserImpl`, `UserSetImpl`, role repositories). Step 2 of the plan will carve these into the future `user-domain` and `user-jpa` modules while leaving authentication services in `platform-identity`.
- Command infrastructure (`DefaultCommandHandler`) resides in `platform-identity` because of identity-specific rollback rules; see §6.2 for the planned extraction into `command-core`.
