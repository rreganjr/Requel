...
|-------|------------------------------|-----------------------------|------------|
| Foundation | `platform-core`              | Move shared exceptions (`RequelException`, `EntityValidationException`, `EntityLockException`, `NoSuchEntityException`), `Describable`, `NamedEntity`, `OrganizedEntity` (after decoupling from `User`), `CreatedEntity` (adjusted to reference a lightweight `UserReference`). Also relocate `SystemInitializer` & `AbstractSystemInitializer` to new `com.rreganjr.platform.bootstrap`. **Status:** already hosts `DomainObjectWrapper`, `EntityProxyInterceptor`, etc. Kept consolidated (command/validation/repository abstractions stay here) because splitting them further would add POM churn and build overhead without delivering meaningful dependency isolation yet. | none |
| Foundation | `platform-identity`          | Minimal identity API (`com.rreganjr.platform.identity.User/Role`), authentication services, password hashing, plus documentation for bridging into the Requel domain. **Status:** user domain and JPA implementations extracted in Oct 2025; module now only exposes identity primitives and shared exceptions. | `platform-core` |
| Domain | `user-domain`                | `com.rreganjr.requel.user` interfaces, DTOs, exceptions. Convert `CreatedEntity` dependency to the new `UserReference` abstraction. **Status:** module created; hosts legacy interfaces/exceptions (several classes retain JPA annotations pending further refactors). | `platform-core`, `platform-identity` |
| Domain | `user-jpa`                   | `com.rreganjr.requel.user.jpa` (renamed from `.impl`) entities and repository adapters. Depends on `user-domain`, `repository-core`. **Status:** module created; contains the former `.impl` entities, repositories, command impls, and `User2UserImplAdapter`. | `user-domain`, `repository-core`, `platform-identity` |
...
| Domain | `annotation-domain`          | `com.rreganjr.requel.annotation` APIs plus registries for annotatable/grouping types; keeps annotation model free of project/user persistence details. | `platform-core`, `platform-identity` |
| Domain | `annotation-jpa`             | Annotation JPA entities, repositories, and import/export helpers; owns Hibernate Any mappings and annotation link registry implementations. | `annotation-domain`, `user-jpa`, `platform-core`, `platform-identity`, `dictionary-jpa`, `utils-jaxb` |
