...
public class GoalEditorPanel extends AbstractRequelProjectEditorPanel {
	private static final Log log = LogFactory.getLog(GoalEditorPanel.class);
	...
	public String getTitle() {
		if (getGoal() != null) {
			String msgPattern = getResourceBundleHelper(getLocale()).getString(
					...
					getResourceBundleHelper(getLocale()).getString(Panel.PROP_PANEL_TITLE, "Goal: {0}"));
			...
			String msg = getResourceBundleHelper(getLocale()).getString(
					...
					getResourceBundleHelper(getLocale()).getString(Panel.PROP_PANEL_TITLE, "New Goal"));
			return msg;
		}
	}
	...
	private Component getRelationsTable() {
		...
		if ((getGoal() != null) && !isReadOnlyMode()) {
			...
			openEditorButton.setStyleName(Panel.STYLE_NAME_DEFAULT);
